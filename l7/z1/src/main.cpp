...
void zadanie(vector<worker> *w){
	...
	// Но для того чтобы посмотреть, что под копотом этих функций придется сходить
	// в мой github: https://guthub.com/NyaHub/labs.git
	// Ах да, часть магии происходит выше, но это скорее подготовка к этому.
	// Кстати, я забыл про должность человеков, но я думаю масштаб проекта перекроет
